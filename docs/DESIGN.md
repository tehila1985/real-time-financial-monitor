...
> **Why this wasn't asked for, and why it's here anyway:** the assignment never mentions abuse protection. But this endpoint accepts financial data from "an external system" (§FR4) with zero authentication — the one thing standing between it and being flooded is nothing. A few lines of already-built-in middleware closing an obvious gap is exactly the kind of proportionate, unprompted judgment call the assignment's "architectural thinking" goal rewards, as long as it doesn't touch what's actually required.
> **Why the limit is 200, not something tighter:** it must sit comfortably *above* the "100 transactions arrive quickly" NFR (§16) — that scenario is the one thing this system is explicitly graded on handling smoothly, so the rate limiter must never be the thing that makes it fail. Same reasoning already used for the retention cap (§13): the number itself isn't significant, staying clear of the burst scenario is.
> **Why a global window, not partitioned per-IP:** per-IP partitioning is the textbook production answer, but this system has no concept of client identity at all (no auth, no API keys) — an IP is trivially spoofable and just as trivially shared (NAT, a corporate proxy), so partitioning on it here would be a false sense of granularity, not real protection. A single global window is simpler, fully deterministic to test, and honest about what it actually is: a blunt circuit-breaker against an accidental or careless flood, not per-client fairness.
> **Why not queuing instead of immediately rejecting (`QueueLimit = 0`):** queuing would delay a caller rather than reject them — for a real-time ingestion endpoint, a delayed write is arguably worse than an honest, immediate `429` the caller can retry.
> **Trade-off:** one misbehaving caller can exhaust the shared budget for everyone else until the window resets. Acceptable here — there is no multi-tenant scenario to protect between, only "the whole system vs. an accidental flood."
> **A real limitation, found in code review and worth being upfront about:** this counter is **in-memory, per process** — each backend pod tracks its own independent window. At `replicas: 2` (§19, the exact deployment this project ships), the *effective* system-wide budget behind the K8s Service is closer to `PermitLimit × replica count`, not the single number configured. This doesn't threaten the burst NFR (it only ever makes the limiter *more* permissive, never less), but it does mean the "protects against a flood" framing above is weaker than one global number implies once there's more than one pod. **Why this isn't fixed here:** a real fix needs a shared counter (e.g., Redis-backed, via the same backplane ADR 0001 already introduces) — solving that is legitimate scope for a production rollout, not for this MVP's rate limiter, whose actual job is closing an obvious "zero protection" gap, not implementing distributed rate limiting.
...
|---|---|---|---|---|---|
| `backend-deployment.yaml` | **2** (deliberately exposes the multi-pod problem — §20) | 8080 | `ASPNETCORE_ENVIRONMENT`, `Storage__RetentionCap` | liveness `GET /health`, readiness `GET /health/ready` (includes Redis reachability — see §20's decision box) | — |
...
See **[ADR 0001](adr/0001-distributed-sync-redis-backplane.md)** for the full analysis — including a sequence diagram of the exact fix in action. Summary:
...
- **Known limitation, not fixed by this:** the backplane synchronizes only the live SignalR broadcast. `IStorage` remains a separate, unsynchronized instance per pod, so `GET /api/transactions` can still return different results depending on which pod the Kubernetes Service happens to route it to. See the ADR's "Scope" section for why this is left as a documented limitation rather than fixed here.
...
> **Decision: readiness (`/health/ready`) reports on the Redis backplane's actual reachability; liveness (`/health`) deliberately does not — and neither does anything at all when Redis isn't configured.**
> **The gap this closes:** the default ASP.NET Core health check (`AddHealthChecks()` with no checks added) only proves the process is running. In a deployment where the whole point of Redis is to keep pods synchronized, a pod that's lost its Redis connection is still "alive" by that definition — but it has silently regressed to the exact single-pod-isolated behavior the backplane exists to fix. A readiness probe pointed at the trivial check would keep routing traffic to it anyway.
> **Why this is split into two endpoints instead of one `/health` used for both probes (the mistake this decision specifically avoids):** liveness answers "would restarting this pod help?" — and restarting the backend does nothing to fix a Redis outage, so tying liveness to Redis would turn a brief, transient Redis blip into a needless restart loop across both replicas at once. Readiness answers a different question — "should this pod receive traffic right now?" — where "no, its backplane is down" is exactly the right answer. `RedisHealthCheck` is registered with an explicit `"ready"` tag; `/health` maps with `Predicate = _ => false` (run zero checks, always Healthy), `/health/ready` maps with `Predicate = check => check.Tags.Contains("ready")`.
> **Why conditional on `Redis:ConnectionString` being set, same as the backplane itself:** local `dotnet run` and the `WebApplicationFactory` integration tests never configure Redis (§17) — they shouldn't start failing on a dependency they were never meant to have. `RedisHealthCheck` (`Health/RedisHealthCheck.cs`) is only registered inside the same `if` block that wires `AddStackExchangeRedis`, so `/health/ready` behaves identically to `/health` (checks nothing, always Healthy) in every environment without Redis.
> **Why a dedicated `IConnectionMultiplexer` singleton instead of a new connection per check:** SignalR's own backplane connection is internal to it, not exposed via DI — reusing it wasn't an option. A multiplexer is meant to be long-lived (StackExchange.Redis's own guidance), so one shared instance for health reporting, not one per probe hit.
> **Why registering it as a lazy DI factory rather than connecting eagerly at startup:** if Redis isn't reachable yet the moment the very first `/health/ready` call resolves it (a real possibility with `docker-compose.yml`'s ordering-only `depends_on`), the standard DI container does not cache that failure — it retries the factory on the next resolution. **Verified, not assumed:** stopped the `redis` container under `docker compose`, confirmed `/health/ready` immediately returned `503` while `/health` kept returning `200`, restarted `redis`, confirmed `/health/ready` returned to `200` on the very next request with no backend restart.
